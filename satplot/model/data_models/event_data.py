...
class EventData(BaseDataModel):
	def __init__(self, e_file: pathlib.Path, timespan: TimeSpan, orbit: Orbit):
		...
		self._latlon = self._interpPositions(
			self._timestamps,
			timespan,
			np.hstack((orbit.lat.reshape(-1, 1), orbit.lon.reshape(-1, 1))),
		)

	def _loadEventFile(
		self, e_file: pathlib.Path
	) -> tuple[
		np.ndarray[tuple[int], np.dtype[np.datetime64]],
		np.ndarray[tuple[int, int], np.dtype[np.float64]],
	]:
		event_timestamps = np.genfromtxt(
			e_file,
			delimiter=",",
			usecols=[0],
			skip_header=1,
		...
		event_descriptions = list(
			np.genfromtxt(e_file, delimiter=",", usecols=[1], skip_header=1, dtype=str)
		)
		return event_timestamps, event_descriptions

	def _interpPositions(
		self,
		t_search: np.ndarray[tuple[int], np.dtype[np.datetime64]],
		...
		return source_pos_array[int_idxs] + (
			source_pos_array[int_idxs + 1] - source_pos_array[int_idxs]
		) * fractional_part.reshape(-1, 1)

	def sliceByTimespanIdx(
		self, timespan_idx: int
	) -> tuple[
		np.ndarray[tuple[int], np.dtype[np.bool_]], np.ndarray[tuple[int], np.dtype[np.bool_]]
	]:
