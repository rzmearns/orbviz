...
class EventData(BaseDataModel):
	def __init__(self, e_file:pathlib.Path, timespan:TimeSpan, orbit:Orbit):
		...
		self._raw_timestamps, self._descriptions = self._loadEventFile(e_file)
	...
	def _loadEventFile(self, e_file: pathlib.Path) -> tuple[np.ndarray[tuple[int], np.dtype[np.datetime64]], np.ndarray[tuple[int,int],np.dtype[np.float64]]]:
		...
		event_descriptions = list(np.genfromtxt(e_file, delimiter=',', usecols=[1],skip_header=1))
	...
	def _sliceByTimespanIdx(self, timespan_idx:int) -> tuple[np.ndarray[tuple[int],np.dtype[bool]],
															np.ndarray[tuple[int],np.dtype[bool]]]:
		pre_truth = self._timestamps <= self.timespan.asDatetime(timespan_idx)
		post_truth = self._timestamps > self.timespan.asDatetime(timespan_idx)
