...
class SphereImageData(BaseDataModel):
	def __init__(
		...
		**kwargs,
		...
		logger.info(
			"Finished initialising SphereImageData:%s for %s: %snm -> %snm, externally lit: %s",
			self,
	...
	def storeArray(self, arr: np.ndarray) -> None:
		logger.info(
			"Finished loading image array data for %s for %s: %snm -> %snm, externally lit: %s",
			self,
			self.getConfigValue("body_name"),
			self.getConfigValue("wavelength")[0],
			self.getConfigValue("wavelength")[1],
	...
	def getLookupData(self) -> dict[str, tuple[float, float] | str | bool]:
		d = {
			"body_name": self.getConfigValue("body_name"),
	...
	def visibleEarthSunlit(cls):
		return cls(
			pathlib.Path(f"{satplot_paths.data_dir}/earth2D/BlueMarble_hires.jpeg"), "earth", True
		)

	@classmethod
	def visibleEarthEclipsed(cls):
		return cls(
			pathlib.Path(f"{satplot_paths.data_dir}/earth2D/BlackMarble_hires.jpeg"), "earth", False
		)
