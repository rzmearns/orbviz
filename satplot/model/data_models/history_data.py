...
class HistoryData(BaseDataModel):
	...
	def setSupplementalConstellation(
		self, constellation_config: data_types.ConstellationConfig
	) -> None:
	...
	def process(self) -> None:
		...
		if self.getConfigValue("is_pointing_defined"):
			for sc_id, sc_config in (
				self.getConfigValue("primary_satellite_config").getAllSpacecraftConfigs().items()
			):
				self.pointings[sc_id] = HistoricalAttitude(
					self.getConfigValue("pointing_file"), sc_config
				)
			if self.getConfigValue("pointing_defines_timespan"):
				console.send("Loading timespan from pointing file.")
				_timearr = self.pointings[
					self.getConfigValue("primary_satellite_ids")[0]
				].getPointingTimestamps()
		...
		if self.timespan is None or not self.getConfigValue("is_pointing_defined"):
			...
			duration = int(
				(
					self.getConfigValue("timespan_period_end")
					- self.getConfigValue("timespan_period_start")
				).total_seconds()
			)
			timestep = self.getConfigValue("sampling_period")
			logger.debug(
				"Timespan has duration:%ss, timestep:%ss, from %s", duration, timestep, period_start
			)
			# TODO: need field checking here for end before start, etc.
			self.timespan = timespan.TimeSpan(
				period_start, timestep=f"{timestep}S", timeperiod=f"{duration}S"
			)
		...
		self._worker_threads["primary"] = threading.Worker(
			self._propagatePrimaryOrbits,
			self.timespan,
			self.getConfigValue("primary_satellite_ids"),
		)
		...
		if self.getConfigValue("has_supplemental_constellation"):
			...
			self._worker_threads["constellation"] = threading.Worker(
				self._propagateConstellationOrbits,
				self.timespan,
				self.constellation.getConfigValue("satellite_ids"),
			)
			self._worker_threads["constellation"].signals.result.connect(
				self.constellation._storeOrbitData
			)
			self._worker_threads["constellation"].signals.report_finished.connect(
				self._procComplete
			)
		...
		if self.getConfigValue("events_defined"):
			...
			self._worker_threads["events"] = threading.Worker(
				self._loadEvents, self.getConfigValue("events_file"), delay_start=True
			)
			self._worker_threads["primary"].addChainedWorker(
				"events", self._worker_threads["events"]
			)
		...
		self._worker_threads["groundstations"] = threading.Worker(
			self._recalculateGroundStations, delay_start=True
		)
		self._worker_threads["primary"].addChainedWorker(
			"groundstations", self._worker_threads["groundstations"]
		)
	...
	def _propagatePrimaryOrbits(
		self, timespan: timespan.TimeSpan, sat_ids: list[int], running: threading.Flag
	) -> dict[int, orbit.Orbit]:
	...
	def _propagateConstellationOrbits(
		self, timespan: timespan.TimeSpan, sat_ids: list[int], running: threading.Flag
	) -> dict[int, orbit.Orbit]:
		updated_list = updater.updateTLEs(sat_ids)  # noqa: F841
		# TODO: check number of sats updated == number of sats requested (remove above noqa)
		...
		for sat_id in progressbar(sat_ids):
			logger.debug(
				"Checking constellation data processing thread flag %s:%s",
				running,
				running.getState(),
			)
			if not running:
	...
	def _loadEvents(
		self, event_file: pathlib.Path, running: threading.Flag
	) -> dict[int, event_data.EventData]:
		event_data_objs = {}
		for sat_id, orbit_data in self.orbits.items():
			if self.timespan is not None:
				event_data_objs[sat_id] = event_data.EventData(
					event_file, self.timespan, orbit_data
				)
	...
	def _createDataPaneEntries(self):
		self.datapane_data.append(
			{
				"parameter": "Altitude",
				"value": lambda: np.linalg.norm(
					list(self.orbits.values())[0].pos[self.curr_index, :]
				)
				- satplot_constants.R_EARTH,
				"unit": "km",
			}
		)
		self.datapane_data.append(
		...
		self.datapane_data.append(
			{
				"parameter": "Period Perigee",
				"value": lambda: min(
					np.linalg.norm(list(self.orbits.values())[0].pos, axis=1)
					- satplot_constants.R_EARTH
				),
				"unit": "km",
			}
		...
		self.datapane_data.append(
			{
				"parameter": "Lat, Long",
				"value": lambda: (
					list(self.orbits.values())[0].lat[self.curr_index],
					list(self.orbits.values())[0].lon[self.curr_index],
				),
				"unit": "°",
			}
		)
		...
		self.datapane_data.append(
			{
				"parameter": "Velocity",
				"value": lambda: np.linalg.norm(
					list(self.orbits.values())[0].vel[self.curr_index, :]
				),
				"unit": "m/s",
			}
		)
...
class HistoricalAttitude:
	def __init__(
		...
		quat_defn_direction: str = "eci2bf",
	):
		...
		self._attitude_quats: np.ndarray[tuple[int, int], np.dtype[np.float64]] = np.zeros(
			self._sc_raw_quats.shape, dtype=np.float64
		)
		self._sens_attitude_quats: dict[
			tuple[str, str], np.ndarray[tuple[int, int], np.dtype[np.float64]]
		] = {}

		self._cached_sc_idx = np.full((num_samples), False)
		self._attitude_matrix_cache: np.ndarray[tuple[int, int, int], np.dtype[np.float64]] = (
			np.zeros((num_samples, 3, 3), dtype=np.float64)
		)
		self._cached_sens_idx: dict[
			tuple[str, str], np.ndarray[tuple[int], np.dtype[np.bool_]]
		] = {}
		self._sens_attitude_matrix_cache: dict[
			tuple[str, str], np.ndarray[tuple[int, int], np.dtype[np.float64]]
		] = {}
		...
		for suite_name, suite_config in sc_config.getSensorSuites().items():
			for sens_name in suite_config.getSensorNames():
				...
				self._sens_attitude_quats[sens_key] = self._quatArrMult(
					self._attitude_quats, np.tile(sens_bf_quat, (num_samples, 1))
				)
				self._cached_sens_idx[sens_key] = np.full((num_samples), False)
				self._sens_attitude_matrix_cache[sens_key] = np.zeros(
					(num_samples, 3, 3), dtype=np.float64
				)

	def getPointingTimestamps(self) -> np.ndarray[tuple[int], np.dtype[np.datetime64]]:
		return self._timestamps

	def _loadPointingFile(
		self, p_file: pathlib.Path
	) -> tuple[
		...
		pointing_dates = np.genfromtxt(
			p_file,
			delimiter=",",
			usecols=[0],
			skip_header=1,
			converters={0: satplot_conversions.date_parser},
		)
	...
	def getSensorAttitudeQuat(
		self, suite_name: str, sens_name: str, *args: int
	) -> (
		np.ndarray[tuple[int], np.dtype[np.float64]]
		| np.ndarray[tuple[int, int], np.dtype[np.float64]]
	):
	...
	def getSensorAttitudeMatrix(
		self, suite_name: str, sens_name: str, idx: int
	) -> (
		np.ndarray[tuple[int], np.dtype[np.float64]]
		| np.ndarray[tuple[int, int], np.dtype[np.float64]]
	):
