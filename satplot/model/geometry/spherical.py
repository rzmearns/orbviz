...
def smallCircleRadius(
	center_lat: float, center_lon: float, radii_end_lat: float, radii_end_lon: float
) -> float:
	...
	d = 2 * np.arcsin(
		np.sqrt(
			np.sin((lat - lat_0) / 2) ** 2
			+ np.cos(lat) * np.cos(lat_0) * np.sin((lon - lon_0) / 2) ** 2
		)
	)
...
def getSmallCirclePoint(
	great_circle_radius: float, center_lat: float, center_lon: float, param_lat: float
) -> tuple[float, float]:
	...
	s = 2 * np.arcsin(
		np.sqrt(
			(np.sin(d / 2) ** 2 - np.sin((theta - theta_0) / 2) ** 2)
...
def genSmallCircleCenterRadii(
	great_circle_radius: float, center_lat: float, center_lon: float
) -> np.ndarray:
	...
	circle = np.vstack(
		(
			np.hstack((lons1.reshape(-1, 1), lats.reshape(-1, 1))),
...
def genSmallCircleCenterSubtendedAngle(
	subtended_angle: float, center_lat: float, center_lon: float
...
def splitSmallCirclePatch(center_lon, center_lat, lats, lons1, lons2):
	...
	if (
		np.all(lons1 > -180)
		and np.all(lons1 < 180)
		and np.all(lons2 > -180)
		and np.all(lons2 < 180)
	):
		...
		circle = np.vstack(
			(
				np.hstack((lons1.reshape(-1, 1), lats1.reshape(-1, 1))),
