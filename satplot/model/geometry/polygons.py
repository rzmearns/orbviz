...
def generateCircle(
	center: tuple[float, float, float] | nptyping.NDArray,
	radius: float,
	...
	for ii in range(3):
		coords[:, ii] = (
			radius * np.cos(theta) * e1[ii]
			+ radius * np.sin(theta) * e2[ii]
			+ np.zeros(sampling) * e3[ii]
		)
...
def polygonTriangulate(polygon_verts):
	...
	t_data["segments"] = np.hstack(
		(
			np.arange(len(t_data["vertices"]) - 1).reshape(-1, 1),
			np.arange(1, len(t_data["vertices"])).reshape(-1, 1),
		)
	)
...
def isPolygonConvex(polygon_verts):
	...
	cnvx_angles = (
		np.sum(
			(np.roll(rot_verts, shift=-1, axis=0) - rot_verts)
			* (np.roll(polygon_verts, shift=-2, axis=0) - rot_verts),
			axis=1,
		)
		> 0
	)
