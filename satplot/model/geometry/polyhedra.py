...
def calcConeMeshGrid(
	apex: tuple[float, float, float] | nptyping.NDArray,
	height: float,
	axis: tuple[float, float, float] | nptyping.NDArray,
	apex_angle_deg: float,
	axis_sample: int = 3,
	r_sample: int = 2,
	theta_sample: int = 100,
) -> tuple[list[nptyping.NDArray], list[nptyping.NDArray]]:
	...
	X, Y, Z = [
		apex[i]
		+ t2 * e3[i]
		+ (t2 * np.tan(phi)) * np.cos(theta2) * e1[i]
		+ (t2 * np.tan(phi)) * np.sin(theta2) * e2[i]
		for i in [0, 1, 2]
	]
	# circle cap
	X2, Y2, Z2 = [
		apex[i] + e3[i] * height + R * np.sin(theta) * e1[i] + R * np.cos(theta) * e2[i]
		for i in [0, 1, 2]
	]
...
def calcConeMesh(
	...
	coords = calcConePoints(
		apex, height, axis, apex_angle_deg, axis_sample=axis_sample, theta_sample=theta_sample
	)
...
def calcSquarePyramidMesh(
	...
	coords = calcSquarePyramidPoints(
		apex, height, axis, x_angle_deg, y_angle_deg, axis_sample=axis_sample
	)
	hull = scipy.spatial.ConvexHull(coords)
...
def calcSphereMeshGrid(
	center: tuple[float, float, float] | nptyping.NDArray, r: float
) -> list[nptyping.NDArray]:
	R = np.sqrt(r)
...
def calcCylinderMeshGrid(
	end_point: tuple[float, float, float] | nptyping.NDArray,
	height: float,
	axis: tuple[float, float, float] | nptyping.NDArray,
	radius: float,
	axis_sample: int = 3,
	r_sample: int = 2,
	theta_sample: int = 100,
) -> tuple[list[nptyping.NDArray], list[nptyping.NDArray], list[nptyping.NDArray]]:
	R = radius
	...
	# cone
	X, Y, Z = [
		end_point[i] + t2 * e3[i] + R * np.cos(theta2) * e1[i] + R * np.sin(theta2) * e2[i]
		for i in [0, 1, 2]
	]
	# circle cap 1
	X2, Y2, Z2 = [
		end_point[i] + R * np.sin(theta) * e1[i] + R * np.cos(theta) * e2[i] for i in [0, 1, 2]
	]
	# circle cap 2
	X3, Y3, Z3 = [
		end_point[i] + e3[i] * height + R * np.sin(theta) * e1[i] + R * np.cos(theta) * e2[i]
		for i in [0, 1, 2]
	]
...
def calcCylinderMesh(
	...
	coords = calcCylinderPoints(
		end_point, height, axis, radius, axis_sample=axis_sample, theta_sample=theta_sample
	)
	hull = scipy.spatial.ConvexHull(coords)
