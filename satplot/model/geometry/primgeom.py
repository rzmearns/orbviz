...
def unitVector(vector: nptyping.NDArray) -> nptyping.NDArray:
	...
	if len(vector.shape) == 1:
		...
		raise exceptions.InputError(
			f"Vectors of shape {vector.shape} are not supported by unitVector."
		)
...
def angleBetween(v1: nptyping.NDArray, v2: nptyping.NDArray, units: str = "rad") -> float:
	...
	if units == "rad":
		...
		raise NotImplementedError(
			f"{units} is not a valid unit for the angle. units must be either 'rad' or 'deg'."
		)


def seqPointsSignedAngle(
	prev_pnt: nptyping.NDArray,
	curr_pnt: nptyping.NDArray,
	next_pnt: nptyping.NDArray,
	norm: nptyping.NDArray = Z,
) -> float:
	"""
...
def vectorSignedAngle(
	v1: nptyping.NDArray, v2: nptyping.NDArray, norm: nptyping.NDArray = Z
) -> float:
	"""
...
def orthogonalProjection(v1: nptyping.NDArray, v2: nptyping.NDArray) -> nptyping.NDArray:
	...
	if v1.shape[-1] != 3 or v2.shape[-1] != 3:
		...
		raise exceptions.DimensionError(
			"Orthogonal projection only defined for 3D vectors or lists of 3D vectors!"
		)
	if v1.shape != v2.shape:
		if v2.shape != (3,):
			logger.error(
				"v2 must be either a single vector, or a list of vectors the same length as v1."
			)
			raise exceptions.DimensionError(
				"v2 must be either a single vector, or a list of vectors the same length as v1."
			)
...
def is_point_on_plane(point: nptyping.NDArray, coeff: nptyping.NDArray) -> bool:
	...
	bool
	"""
	return np.isclose(coeff[0] * point[0] + coeff[1] * point[1] + coeff[2] * point[2], -coeff[3])
