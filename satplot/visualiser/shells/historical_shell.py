...
class HistoricalShell(base_shell.BaseShell):
	def __init__(
		...
		self._addContext(
			...
			history_configuration_context.HistoryConfigurationContext(
				"configuration-history", self.window, self.data["history"]
			),
		)
		self._addContext(
			"3D-history",
			history3d_context.History3DContext(
				"3D-history", self.window, self.data["history"], self.data["groundstations"]
			),
		)
		self._addContext(
			"2D-history",
			history2d_context.History2DContext(
				"2D-history",
				self.window,
				self.data["history"],
		...
		self._addContext(
			"sensors-view-history",
			sensor_views_context.SensorViewsContext(
				"sensors-view-history",
				self.window,
				self.data["history"],
				self.data["groundstations"],
