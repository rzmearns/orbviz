...
class History3DContext(base.BaseContext):
	...
	def saveGif(
		...
		file: pathlib.Path,
		loop=True,
		camera_adjustment_data={"az_start": 0, "el_start": 0, "az_range": 0, "el_range": 0},
		start_index=0,
		end_index=-1,
	):
		...
		for ii in range(start_idx, end_idx):
			...
			self.canvas_wrapper.view_box.camera.elevation = (
				start_elevation - ii * elevation_step_angle
			)
	...
	def setupGIFDialog(self):
		dflt_camera_setup = {
			"az_start": self.canvas_wrapper.view_box.camera.azimuth,
