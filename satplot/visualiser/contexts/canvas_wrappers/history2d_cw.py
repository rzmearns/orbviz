...
class History2DCanvasWrapper(BaseCanvas):
	def __init__(
		self, w: int = 1200, h: int = 600, keys: str = "interactive", bgcolor: str = "white"
	):
		...
		self.view_box.camera = RestrictedPanZoom.RestrictedPanZoomCamera(
			limits=(0, IMAGE_SHAPE[0], 0, IMAGE_SHAPE[1])
		)
	...
	def _buildAssets(self) -> None:
		...
		self.assets["groundstations"] = groundstations.GroundStation2DAsset(
	...
	def setModel(
		...
		gs_data: GroundStationCollection,
	...
	def modelUpdated(self) -> None:
		...
		self.assets["groundstations"].setSource(
			self.data_models["groundstations"], self.data_models["history"]
		)
	...
	def centerCameraEarth(self) -> None:
		if self.canvas is None:
			logger.warning(
				"Canvas has not been set for History2D Canvas Wrapper. No camera to center"
			)
	...
	def mapScreenPosToWorld(self, screen_pos):
		...
		world_y = self.vb_max_extents[1] - (
			(self.vb_max_extents[1] - curr_screen_rect.top)
			+ screen_pos[1] / canvas_height * curr_screen_rect.height
		)
	...
	def mapWorldPosToScreen(self, world_pos):
		...
		screen_pos_x = (
			(world_pixels_x - curr_screen_rect.left) * canvas_width / curr_screen_rect.width
		)
		screen_pos_y = (
			(
				(self.vb_max_extents[1] - world_pixels_y)
				- (self.vb_max_extents[1] - curr_screen_rect.top)
			)
			* canvas_height
			/ curr_screen_rect.height
		)
