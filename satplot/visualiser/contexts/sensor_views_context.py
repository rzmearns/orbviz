...
class SensorViewsContext(BaseContext):
	data_type = [data_types.DataType.HISTORY, data_types.DataType.PLANETARYRAYCAST]
	...
	def _validateDataType(self) -> None:
		if (
			self.data["history"] is not None
			and self.data["history"].getType() not in self.data_type
		):
			console.sendErr(
				f"Error: history3D context has wrong data type: {self.data['history'].getType()}"
			)
			console.sendErr(f"\t should be one of: {self.data_type}")
		if (
			self.data["raycast_src"] is not None
			and self.data["raycast_src"].getType() not in self.data_type
		):
			console.sendErr(
				f"Error: history3D context has wrong data type: {self.data['raycast_src'].getType()}"
			)
			console.sendErr(f"\t should be one of: {self.data_type}")
	...
	def generateSensorFullRes(
		self, view_id: int, sc_id: int, suite_key: str, sens_key: str
	) -> None:
		logger.debug(
			"Generating Full Res for view %s: %s - %s - %s", view_id, sc_id, suite_key, sens_key
		)
		img_data, mo_data, moConverterFunction, img_metadata = (
			self.canvas_wrapper.generateSensorFullRes(sc_id, suite_key, sens_key)
		)
		satplot_dialogs.fullResSensorImageDialog(
			img_data, mo_data, moConverterFunction, img_metadata
		)
