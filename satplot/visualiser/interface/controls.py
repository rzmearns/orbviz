...
class PrimaryConfig(QtWidgets.QWidget):
	...
	def __init__(self, parent: QtWidgets.QWidget | None = None) -> None:
		...
		self.prim_config_selector = widgets.FilePicker(
			"Configuration File",
...
class TimePeriodConfig(QtWidgets.QWidget):
	def __init__(self, parent: QtWidgets.QWidget | None = None) -> None:
		...
		# Configuration Widgets
		self.period_start = widgets.DatetimeEntry(
			"Period Start:",
			dt.datetime.now(tz=dt.timezone.utc) - dt.timedelta(seconds=1.5 * 60 * 60),
		)
		self.period_end = widgets.DatetimeEntry(
			"Period End:",
			(dt.datetime.now(tz=dt.timezone.utc) + dt.timedelta(seconds=1.5 * 60 * 60)),
		)
...
class HistoricalPointingConfig(QtWidgets.QWidget):
	def __init__(self, *args, **kwargs):
		...
		# Configuration widgets
		dflt_config_file = satplot_paths.pnt_dir.joinpath(
			"20240108_test_quaternion_X_ECI_parallel_Z_Zenith.csv"
		)
		self._pointing_file_selector = widgets.FilePicker(
			"Pointing File",
			dflt_file=dflt_config_file.name,
			dflt_dir=dflt_config_file.parent,
			save=False,
		...
		self.pointing_file_inv_toggle = widgets.LabelledSwitch(
			labels=("BF->ECI", "ECI->BF"), dflt_state=True
		)
...
class HistoricalEventConfig(QtWidgets.QWidget):
	def __init__(self, *args, **kwargs):
		...
		self.events_config_selector = widgets.FilePicker(
			"Events File",
			dflt_file=dflt_config_file.name,
			dflt_dir=dflt_config_file.parent,
			save=False,
...
class ConstellationControls(QtWidgets.QWidget):
	def __init__(self, *args, **kwargs):
		...
		self.const_config_selector = widgets.FilePicker(
			"Configuration File",
			dflt_file=dflt_config_file.name,
			dflt_dir=dflt_config_file.parent,
			save=False,
...
class OptionConfigs(QtWidgets.QWidget):
	...
	def buildWidgetPane(self, cw_assets, root_layout):
		...
		for root_asset_key, root_asset in cw_assets.items():
			...
			root_asset_w_dict = self.recursiveBuildOptionWidgets(
				root_asset, self.opt_sections[opt_section_title]["cb"]
			)
	...
	def recursiveBuildOptionWidgets(self, curr_asset, parent_section) -> dict:
		...
		if hasattr(curr_asset, "assets"):
			...
			for sub_asset_key, sub_asset in curr_asset.assets.items():
				logger.debug(
					"Creating collapsible section for sub asset %s belonging to %s",
					sub_asset,
					curr_asset,
				)
	...
	def _buildOptionWidget(self, opt_key: str, opt_cnfg: dict) -> QtWidgets.QWidget:
		...
		if opt_cnfg["type"] == "boolean":
			try:
				widget = widgets.ToggleBox(label_str, opt_cnfg["value"])
			...
			try:
				widget = widgets.ColourPicker(label_str, opt_cnfg["value"])
				widget.add_connect(opt_cnfg["callback"])
				logger.debug("Adding option callback %s to %s", opt_cnfg["callback"], opt_key)
			except:
				logger.warning("Can't make widget %s for asset %s", label_str, opt_key)
			...
			try:
				widget = widgets.ValueSpinner(label_str, opt_cnfg["value"])
				widget.add_connect(opt_cnfg["callback"])
				logger.debug("Adding option callback %s to %s", opt_cnfg["callback"], opt_key)
			except:
				logger.warning("Can't make widget %s for asset %s", label_str, opt_key)
			...
			try:
				widget = widgets.ValueSpinner(label_str, opt_cnfg["value"], integer=False)
				widget.add_connect(opt_cnfg["callback"])
				logger.debug("Adding option callback %s to %s", opt_cnfg["callback"], opt_key)
			except:
				logger.warning("Can't make widget %s for asset %s", label_str, opt_key)
			...
			try:
				widget = widgets.ValueSpinner(label_str, opt_cnfg["value"], fraction=True)
				widget.add_connect(opt_cnfg["callback"])
				logger.debug("Adding option callback %s to %s", opt_cnfg["callback"], opt_key)
			except:
				logger.warning("Can't make widget %s for asset %s", label_str, opt_key)
			...
			try:
				widget = widgets.BasicOptionBox(
					label_str, dflt_option=opt_cnfg["value"], options_list=opt_cnfg["options"]
				)
				widget.add_connect(opt_cnfg["callback"])
				logger.debug("Adding option callback %s to %s", opt_cnfg["callback"], opt_key)
			except:
		...
		for opt_section_title, opt_section in self.opt_sections.items():
			...
			for opt_key, opt in opt_section["opts"].items():
				if not isinstance(opt_section["widget"], widgets.CollapsibleSection):
					state[opt_section_title][opt_key] = opt["widget_data"][
						"widget"
					].prepSerialisation()
	...
	def deSerialise(self, state: dict[str, Any]) -> None:
		for opt_section_title, opt_section in state.items():
			if opt_section_title not in self.opt_sections.keys():
				logger.warning(
					"%s not a recognised context configuration section", opt_section_title
				)
				continue
			for opt_key, opt_serialisation in opt_section.items():
				if opt_key not in self.opt_sections[opt_section_title]["opts"].keys():
					logger.warning(
						"%s not a recognised option for context configuration section %s",
						opt_key,
						opt_section_title,
					)
					continue
				self.opt_sections[opt_section_title]["opts"][opt_key]["widget_data"][
					"widget"
				].deSerialise(opt_serialisation)
...
class SensorViewConfigs(QtWidgets.QWidget):
	...
	def __init__(self, num_views: int = 4, parent: QtWidgets.QWidget | None = None) -> None:
		...
		for ii in range(self._num_views):
			self.view_spacecraft_selectors.append(
				widgets.OptionBox(f"View {ii + 1} Spacecraft:", options_list=[])
			)
			self.view_sensor_selectors.append(
				widgets.OptionBox(f"View {ii + 1} Sensor:", options_list=[])
			)
	...
	def onGenerateSelection(self, view_id):
		if (
			self.view_spacecraft_selectors[view_id].getCurrentIndex() is None
			or self.view_sensor_selectors[view_id].getCurrentIndex() is None
		):
			return
		else:
...
class Toolbar(QtWidgets.QWidget):
	...
	def addButtons(self):
		...
		for key, action in self.action_dict.items():
			...
			if "all".lower() in action_context_list and action["button_icon"] is not None:
				self.button_dict[key] = QtWidgets.QAction(
					QtGui.QIcon(action["button_icon"]), action["tooltip"], self
				)
		...
		for key, action in self.action_dict.items():
			action_context_list = [ac.lower() for ac in action["contexts"]]
			if (
				self.context_name.lower() in action_context_list
				and "all" not in action_context_list
				and action["button_icon"] is not None
			):
				self.button_dict[key] = QtWidgets.QAction(
					QtGui.QIcon(action["button_icon"]), action["tooltip"], self
				)
	...
	def addToWindow(self):
		if self.parent_window is not None:
			...
			logger.error(
				"Can't add toolbar to window, context {context_name} doesn't have a window yet."
			)
			raise ValueError(
				"Can't add toolbar to window, context {context_name} doesn't have a window yet."
			)
		...
		for key, action in self.action_dict.items():
			...
			if "all".lower() in action_context_list:
				if action["containing_menu"] not in self.menus.keys():
					self.menus[action["containing_menu"]] = self.menubar.addMenu(
						action["containing_menu"].capitalize()
					)
				self.button_dict[key] = QtWidgets.QAction(
					QtGui.QIcon(action["button_icon"]), action["menu_item"], self
				)
				self.button_dict[key].setStatusTip(action["tooltip"])
				self.button_dict[key].setCheckable(action["toggleable"])
				if action["callback"] is not None:
