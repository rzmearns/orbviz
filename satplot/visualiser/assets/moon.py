...
class Moon3DAsset(base_assets.AbstractAsset):
	...
	def setSource(self, *args, **kwargs) -> None:
		if type(args[0]) is not orbit.Orbit:
			logger.error(
				"setSource() of %s requires an %s as value of dict from args[0], not: {type(args[0])}",
				self,
				orbit.Orbit,
			)
			raise TypeError
	...
	def _createVisuals(self) -> None:
		self.visuals["moon"] = scene.visuals.Sphere(
			radius=self.opts["moon_sphere_radius_kms"]["value"],
			method="latitude",
	...
	def recomputeRedraw(self) -> None:
		...
		if self.isStale():
			moon_pos = self.opts["moon_distance_kms"]["value"] * pg.unitVector(
				self.data["curr_pos"]
			)
	...
	def _setDefaultOptions(self) -> None:
		...
		self._dflt_opts["moon_sphere_colour"] = {
			"value": (61, 61, 61),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["moon_distance_kms"] = {
			"value": 15000,
			"type": "number",
			"help": "",
			"static": True,
		...
		self._dflt_opts["moon_sphere_radius_kms"] = {
			"value": 786,
			"type": "number",
			"help": "",
			"static": True,
	...
	def setMoonSphereColour(self, new_colour: tuple[float, float, float]) -> None:
		logger.debug(
			"Changing moon sphere colour %s -> %s",
			self.opts["moon_sphere_colour"]["value"],
			new_colour,
		)
		...
		self.visuals["moon"].mesh._meshdata.set_face_colors(
			np.tile(colours.normaliseColour(new_colour), (n_faces, 1))
		)
		self.visuals["moon"].mesh._meshdata.set_vertex_colors(
...
class Moon2DAsset(base_assets.AbstractAsset):
	...
	def setSource(self, *args, **kwargs):
		...
		if type(first_sat_orbit) is not orbit.Orbit:
			logger.error(
				"data source for %s is not an orbit.Orbit, can't extract moon location data", self
			)
			raise TypeError

		lat, lon, alt = pymap3d.eci2geodetic(
			first_sat_orbit.moon_pos[:, 0],
	...
	def _setDefaultOptions(self):
		...
		self._dflt_opts["plot_moon_marker"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["moon_marker_colour"] = {
			"value": (159, 159, 159),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["moon_marker_size"] = {
			"value": 30,
			"type": "number",
			"help": "",
			"static": True,
	...
	def setMoonMarkerColour(self, new_colour):
		logger.debug(
			"Changing moon marker colour %s -> %s",
			self.opts["moon_marker_colour"]["value"],
			new_colour,
		)
	...
	def setMoonMarkerSize(self, size):
		logger.debug(
			"Changing moon marker size %s -> %s", self.opts["moon_marker_size"]["value"], size
		)
