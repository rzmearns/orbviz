...
class SensorSuite3DAsset(base_assets.AbstractCompoundAsset):
	...
	def _instantiateAssets(self) -> None:
		...
		for sensor in sensor_names:
			...
			if sens_dict["shape"] == satplot_data_types.SensorTypes.CONE:
				self.assets[sensor] = Sensor3DAsset.cone(
					self.data["sc_id"],
					sensor,
					self.data["name"],
					sens_dict,
				...
				self.assets[sensor] = Sensor3DAsset.squarePyramid(
					self.data["sc_id"],
					sensor,
					self.data["name"],
					sens_dict,
	...
	def setTransform(
		self,
		pos: tuple[float, float, float] | nptyping.NDArray = (0, 0, 0),
		rotation: nptyping.NDArray | None = None,
		quat: nptyping.NDArray | None = None,
	) -> None:
		if self.isStale():
			if rotation is None and quat is None:
				logger.warning(
					"Rotation and quaternion passed to sensor suite: %s cannot both be None",
					self.data["name"],
				)
				raise ValueError(
					f"Rotation and quaternion passed sensor suite: {self.data['name']} cannot both be None"
				)
			if rotation is not None and quat is not None:
				logger.warning(
					"Both rotation and quaternion passed to sensor suite: %s, don't know which one to use",
					self.data["name"],
				)
				raise ValueError(
					"Both rotation and quaternion passed to sensor suite: %s, don't know which one to use",
...
class Sensor3DAsset(base_assets.AbstractSimpleAsset):
	def __init__(
		...
		**kwargs,
		...
		if self.data["type"] is None:
			logger.error(
				"Sensor() should not be called directly, use one of the constructor methods"
			)
			raise ValueError(
				"Sensor() should not be called directly, use one of the constructor methods"
			)
	...
	def _initData(
		...
		sens_type: str,
		sensor_name: str,
		mesh_verts: nptyping.NDArray,
		mesh_faces: nptyping.NDArray,
		bf_quat: nptyping.NDArray,
	...
	def _createVisuals(self) -> None:
		self.visuals["sensor_cone"] = vVisuals.Mesh(
		...
		self.setTransform(
			rotation=Rotation.from_quat(self.data["bf_quat"]).as_matrix().reshape(3, 3)
		)
	...
	def setTransform(
		...
		if self.isStale():
			T = np.eye(4)
			rot_mat = (
				self.data["history_src"]
				.getSCAttitude(self.data["sc_id"])
				.getSensorAttitudeMatrix(
					self.data["parent_suite_name"], self.data["name"], self.data["curr_index"]
				)
			)
			pos = (
				self.data["history_src"]
				.getOrbits()[self.data["sc_id"]]
				.pos[self.data["curr_index"]]
			)
			self.data["vispy_quat"] = (
				self.data["history_src"]
				.getSCAttitude(self.data["sc_id"])
				.getSensorAttitudeQuat(
					self.data["parent_suite_name"], self.data["name"], self.data["curr_index"]
				)
			)
			T[0:3, 0:3] = rot_mat
	...
	def _setDefaultOptions(self) -> None:
		...
		self._dflt_opts["sensor_cone_colour"] = {
			"value": (10, 10, 10),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["sensor_cone_alpha"] = {
			"value": 0.5,
			"type": "float",
			"help": "",
			"static": True,
	...
	def cone(
		...
		mesh_verts, mesh_faces = polyhedra.calcConeMesh(
			(0, 0, 0), sensor_dict["range"], (0, 0, 1), sensor_dict["fov"]
		)
	...
	def squarePyramid(
		...
		mesh_verts, mesh_faces = polyhedra.calcSquarePyramidMesh(
			(0, 0, 0),
			sensor_dict["range"],
			(0, 0, 1),
			sensor_dict["fov"][1],
	...
	def _instantiateAssets(self) -> None:
		...
		for sensor_name in sensor_names:
			...
			if sens_dict["shape"] == satplot_data_types.SensorTypes.CONE:
				...
				self.assets[sensor_name] = Sensor2DAsset(
					self.data["sc_id"],
					sensor_name,
					self.data["name"],
					sens_dict,
...
class Sensor2DAsset(base_assets.AbstractSimpleAsset):
	...
	def _initData(
		...
		sc_id: int,
		parent_suite_name: str,
		bf_quat: tuple[float, float, float, float],
		...
		self.data["lowres_rays_sf"] = self.data["lens_model"].generatePixelRays(
			self.data["lowres"], self.data["fov"]
		)
	...
	def _createVisuals(self) -> None:
		self.visuals["pixel_projection"] = vVisuals.Markers(
			parent=None, scaling=False, antialias=False
		)
		self.visuals["pixel_projection"].set_data(
			pos=self.data["point_cloud"],
			edge_width=1e-9,
	...
	def setTransform(
		...
		if self.isStale() and self.isActive():
			...
			lats, lons = self.data["raycast_src"].rayCastFromSensorFor2D(
				self.data["lowres"], T, self.data["lowres_rays_sf"], self.data["curr_datetime"]
			)
	...
	def _setDefaultOptions(self) -> None:
		...
		self._dflt_opts["sensor_pixel_size"] = {
			"value": 1,
			"type": "number",
			"help": "",
			"static": True,
	...
	def _instantiateAssets(self) -> None:
		...
		for sensor_name in sensor_names:
			...
			if sens_dict["shape"] == satplot_data_types.SensorTypes.CONE:
				...
				self.assets[sensor_name] = SensorImageAsset(
					self.data["sc_id"], sensor_name, self.data["name"], sens_dict, v_parent=None
				)
...
class SensorImageAsset(base_assets.AbstractSimpleAsset):
	def __init__(
		...
		self._setDefaultOptions()
		self._initData(
			sc_id, parent_suite_name, config["bf_quat"], config["resolution"], config["fov"]
		)
		self._instantiateAssets()
		self._createVisuals()
		self.counter = 0
	...
	def _initData(
		...
		self.data["lowres_pix_per_rad"] = self.data["lens_model"].calcPixelAngularSize(
			self.data["lowres"], self.data["fov"]
		)
		self.data["rays_sf"] = self.data["lens_model"].generatePixelRays(
			self.data["res"], self.data["fov"]
		)
		self.data["pix_per_rad"] = self.data["lens_model"].calcPixelAngularSize(
			self.data["res"], self.data["fov"]
		)
		self.data["last_transform"] = np.eye(4)
		self.data["history_src"] = None
	...
	def _createVisuals(self) -> None:
		...
		self.visuals["text"] = vVisuals.Text(
			f"Sensor: {self.data['name']}", color="red", anchor_x="left", anchor_y="bottom"
		)
	...
	def setTransform(
		...
		if self.isStale() and self.isActive():
			...
			self.data["last_transform"] = T
			img_data, mo_data = self.data["raycast_src"].rayCastFromSensor(
				self.data["lowres"],
			...
			data_reshaped = (
				img_data.reshape(self.data["lowres"][1], self.data["lowres"][0], 3) / 255
			)
	...
	def getLowResMOString(self, fractional_pos: tuple[float, float]) -> str:
		pix_pos = (
			int(round(fractional_pos[0] * self.data["lowres"][0])),
			int(round(fractional_pos[1] * self.data["lowres"][1])),
		)
		pos_idx = np.ravel_multi_index(
			(pix_pos[1], pix_pos[0]), (self.data["lowres"][1], self.data["lowres"][0])
		)
	...
	def getFullResMOString(self, mo_data, fractional_pos: tuple[float, float]) -> str:
		pix_pos = (
			int(round(fractional_pos[0] * self.data["res"][0])),
			int(round(fractional_pos[1] * self.data["res"][1])),
		)
		pos_idx = np.ravel_multi_index(
			(pix_pos[1], pix_pos[0]), (self.data["res"][1], self.data["res"][0])
		)
	...
	def _generateLatLonCoords(self, counter):
		...
		lon_fov = lat_fov * 2
		lat = np.linspace(
			lat_center - lat_fov / 2, lat_center + lat_fov / 2, self.data["lowres"][1]
		)
	...
	def _setDefaultOptions(self) -> None:
		self._dflt_opts = {}
		self._dflt_opts["show_sensor_name"] = {
			"value": False,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["highlight_limb"] = {
			"value": False,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["solar_lighting"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["plot_atmosphere"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["atmosphere_height"] = {
			"value": 150,
			"type": "integer",
			"help": "",
			"static": True,
		...
		self._dflt_opts["highlight_height"] = {
			"value": 10,
			"type": "integer",
			"help": "",
			"static": True,
		...
		self._dflt_opts["atmosphere_lit_colour"] = {
			"value": (168, 231, 255),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["atmosphere_eclipsed_colour"] = {
			"value": (23, 32, 35),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["highlight_colour"] = {
			"value": (225, 0, 0),
			"type": "colour",
			"help": "",
			"static": True,
