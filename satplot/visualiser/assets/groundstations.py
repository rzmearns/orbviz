...
class GroundStation3DAsset(base_assets.AbstractAsset):
	...
	def _createVisuals(self):
		# Sun Sphere
		self.visuals["marker"] = scene.visuals.Markers(
			parent=None, scaling=True, antialias=0, spherical=True
		)
		self.visuals["marker"].set_data(
			pos=self.data["coords"],
			edge_width=0,
	...
	def setSource(self, *args, **kwargs):
		...
		if type(args[0]) is not groundstation_data.GroundStationCollection:
			logger.error(
				"setSource() of %s requires a %s as args[1], not: {type(args[1])}",
				self,
				groundstation_data.GroundStationCollection,
			)
			raise TypeError(
				f"setSource() of {self} requires a {groundstation_data.GroundStationCollection} as args[1], not: {type(args[1])}"
			)
	...
	def getScreenMouseOverInfo(self) -> dict[str, Any]:
		...
		if self.groundstations_enabled:
			...
			canvas_pos = (
				self.visuals["marker"].get_transform("visual", "canvas").map(curr_world_pos)
			)
			canvas_pos /= canvas_pos[:, 3:]

			mo_info["screen_pos"] = [
				(canvas_pos[ii, 0], canvas_pos[ii, 1])
				for ii in range(len(self.data["coords"][self.data["curr_index"], :, :]))
			]
			mo_info["world_pos"] = [
				self.data["coords"][self.data["curr_index"], ii, :].reshape(3)
				for ii in range(len(self.data["coords"][self.data["curr_index"], :, :]))
			]
	...
	def _setDefaultOptions(self):
		self._dflt_opts = {}
		self._dflt_opts["plot_groundstations"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["groundstation_marker_colour"] = {
			"value": (243, 243, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["groundstation_marker_size"] = {
			"value": 300,
			"type": "number",
			"help": "",
			"static": True,
		...
		if self.groundstations_enabled:
			self.visuals["marker"].set_data(
				...
				face_color=colours.normaliseColour(
					self.opts["groundstation_marker_colour"]["value"]
				),
			)

	def setGroundStationMarkerColour(self, new_colour):
		logger.debug(
			"Changing GroundStation marker colour %s -> %s",
			self.opts["groundstation_marker_colour"]["value"],
			new_colour,
		)
	...
	def setGroundStationMarkerSize(self, size):
		logger.debug(
			"Changing ground station marker size %s -> %s",
			self.opts["groundstation_marker_size"]["value"],
			size,
		)
	...
	def setMarkerStyle(self, option_idx):
		...
		logger.debug(
			"Changing groundstation marker style %s -> %s",
			self.opts["groundstation_marker_style"]["value"],
			new_style,
		)
...
class GroundStation2DAsset(base_assets.AbstractAsset):
	...
	def _recreateOTHCircleVisuals(self):
		...
		for ii in range(len(self.data["oth_edges1"])):
			v = polygon_visuals.FastPolygon(
				self.data["oth_edges1"][ii],
			...
			v2 = polygon_visuals.FastPolygon(
				self.data["oth_edges2"][ii],
				color=colours.normaliseColour(self.opts["over_the_horizon_circle_colour"]["value"]),
				border_color=colours.normaliseColour(
					self.opts["over_the_horizon_circle_colour"]["value"]
				),
	...
	def setSource(self, *args, **kwargs):
		...
		if self.groundstations_enabled:
			...
			for ii, station in enumerate(stations.values()):
				...
				edge1_data, edge2_data, split = self.calcOTHCircle(
					self.data["min_elevations"][ii], self.data["coords"][ii]
				)
			...
			for ii in range(len(stations.values())):
				if self.data["oth_circle_splits"][ii]:
					self.visuals["oth_circles1"][ii].opacity = self.opts[
						"over_the_horizon_circle_alpha"
					]["value"]
					self.visuals["oth_circles2"][ii].opacity = self.opts[
						"over_the_horizon_circle_alpha"
					]["value"]
				else:
					self.visuals["oth_circles1"][ii].opacity = (
						self.opts["over_the_horizon_circle_alpha"]["value"] / 2
					)
					self.visuals["oth_circles2"][ii].opacity = (
						self.opts["over_the_horizon_circle_alpha"]["value"] / 2
					)
	...
	def getScreenMouseOverInfo(self) -> dict[str, Any]:
		...
		if self.groundstations_enabled:
			...
			mo_info["world_pos"] = [
				(self.data["coords"][ii, 1], self.data["coords"][ii, 0])
				for ii in range(len(self.data["coords"]))
			]
			mo_info["strings"] = self.data["strings"]
			mo_info["objects"] = [self for ii in range(len(self.data["coords"]))]
		return mo_info
	...
	def _setDefaultOptions(self):
		...
		self._dflt_opts["groundstation_marker_style"] = {
			"value": "disc",
			"type": "option",
			"options": [
				"disc",
	...
	def setOTHCircleAlpha(self, alpha):
		...
		logger.debug(
			"Changing groundstation OTH alpha %s -> %s",
	...
	def setOTHCircleColour(self, new_colour):
		logger.debug(
			"Changing groundstation OTH colour %s -> %s",
			self.opts["over_the_horizon_circle_colour"]["value"],
			new_colour,
		)
		self.opts["over_the_horizon_circle_colour"]["value"] = new_colour
		for ii in range(len(self.visuals["oth_circles1"])):
			self.visuals["oth_circles1"][ii].color = colours.normaliseColour(
				self.opts["over_the_horizon_circle_colour"]["value"]
			)
			self.visuals["oth_circles2"][ii].color = colours.normaliseColour(
				self.opts["over_the_horizon_circle_colour"]["value"]
			)
			self.visuals["oth_circles1"][ii].border_color = colours.normaliseColour(
				self.opts["over_the_horizon_circle_colour"]["value"]
			)
			self.visuals["oth_circles2"][ii].border_color = colours.normaliseColour(
				self.opts["over_the_horizon_circle_colour"]["value"]
			)
	...
	def setOTHCircleVisibility(self, state):
		...
		for ii in range(len(self.visuals["oth_circles1"])):
			self.visuals["oth_circles1"][ii].visible = self.opts["plot_over_the_horizon_circle"][
				"value"
			]
			self.visuals["oth_circles2"][ii].visible = self.opts["plot_over_the_horizon_circle"][
				"value"
			]

	def calcOTHCircle(
		self, min_elevation: float, center: np.ndarray[tuple[int], np.dtype[np.float64]]
	):
		...
		lats, lons1, lons2 = spherical_geom.genSmallCircleCenterSubtendedAngle(
			phi * 2, center[1], center[0]
		)
		circle1, circle2 = spherical_geom.splitSmallCirclePatch(
			center[0], center[1], lats, lons1, lons2
		)
