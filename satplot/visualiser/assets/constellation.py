...
class Constellation(base_assets.AbstractAsset):
	...
	def setSource(self, *args, **kwargs) -> None:
		...
		if type(sats_dict) is not dict:
			logger.error(
				"args[0] of Constellation.setSource() should be a dictionary of satellites "
			)
			raise TypeError
		for v in sats_dict.values():
			if type(v) is not orbit.Orbit:
				logger.error(
					"setSource() of %s requires an %s as value of dict from args[0], not: %s",
					self,
					orbit.Orbit,
					type(first_sat_orbit),
				)
				raise TypeError
		...
		self.data["coords"] = np.zeros(
			(self.data["num_sats"], len(list(sats_dict.values())[0].pos), 3)
		)
		for ii in range(self.data["num_sats"]):
			self.data["coords"][ii, :, :] = list(sats_dict.values())[ii].pos
		self.data["beam_height"] = self._calcBeamHeight(
			self.data["beam_angle_deg"] / 2, np.linalg.norm(list(sats_dict.values())[0].pos[0, :])
		)

		if self.assets["beams"] is not None:
			self.assets["beams"].setSource(
				self.data["num_sats"],
				self.data["coords"],
	...
	def _instantiateAssets(self) -> None:
		...
		if satplot.gl_plus:
			self.assets["beams"] = InstancedConstellationBeams(
				name=f"{self.data['name']}_beams", v_parent=self.data["v_parent"]
			)
		else:
			self.assets["beams"] = ConstellationBeams(
				name=f"{self.data['name']}_beams", v_parent=self.data["v_parent"]
			)

	def _createVisuals(self) -> None:
		self.visuals["markers"] = scene.visuals.Markers(
			scaling=True, edge_color="white", symbol="o", antialias=0, parent=None
		)
	...
	def getScreenMouseOverInfo(self) -> dict[str, Any]:
		...
		for ii in range(self.data["num_sats"]):
			...
			canvas_pos = (
				self.visuals["markers"].get_transform("visual", "canvas").map(curr_world_pos)
			)
	...
	def _setDefaultOptions(self) -> None:
		...
		self._dflt_opts["plot_constellation"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["constellation_colour"] = {
			"value": (0, 255, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["plot_constellation_position_markers"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["constellation_position_marker_size"] = {
			"value": 250,
			"type": "number",
			"help": "",
			"static": True,
		...
		self._dflt_opts["constellation_position_marker_colour"] = {
			"value": (0, 255, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["plot_constellation_beams"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
	...
	def _updateMarkers(self):
		...
		self.visuals["markers"].set_data(
			pos=self.data["coords"][:, self.data["curr_index"], :].reshape(-1, 3),
			size=self.opts["constellation_position_marker_size"]["value"],
			face_color=colours.normaliseColour(
				(
					self.opts["constellation_colour"]["value"][0] / darkness_factor,
	...
	def _calcBeamHeight(
		self, half_beam_angle: float, vector_length: float | np.floating[Any]
	) -> float:
...
class InstancedConstellationBeams(base_assets.AbstractAsset):
	...
	def setSource(self, *args, **kwargs) -> None:
		...
		if type(args[0]) is not int:
			logger.error(
				"InstancedConstellationBeams.setSource() args[0]:num_sats is not an int -> %s",
				args[0],
			)
		...
		if type(args[1]) is not np.ndarray:
			logger.error(
				"InstancedConstellationBeams.setSource() args[1]:coords is not an ndarray -> %s",
				args[1],
			)
		...
		if type(args[2]) is not int:
			logger.error(
				"InstancedConstellationBeams.setSource() args[2]:curr_index is not an int -> %s",
				args[2],
			)
		...
		if type(args[3]) is not float and type(args[3]) is not np.float64:
			logger.error(
				"InstancedConstellationBeams.setSource() args[3]:beam_height is not a float -> %s",
				args[3],
			)
		...
		if args[4] >= 180 or args[4] <= 0:
			logger.error(
				"InstancedConstellationBeams.setSource() args[4]:beam_angle_deg must be be 0<angle<180 -> %s",
				args[4],
			)
	...
	def _createVisuals(self) -> None:
		instance_colours = np.tile(
			colours.normaliseColour(self.opts["beams_colour"]["value"]), (self.data["num_sats"], 1)
		)
		instance_positions = self.data["coords"][:, self.data["curr_index"], :].reshape(-1, 3)

		instance_transforms = np.zeros((self.data["num_sats"], 3, 3))
		for ii in range(self.data["num_sats"]):
			beam_axis = -1 * pg.unitVector(
				self.data["coords"][ii, self.data["curr_index"], :]
			).reshape(1, 3)
			instance_transforms[ii, :, :] = np.linalg.inv(
				Rotation.align_vectors(self.data["start_beam_vec"], beam_axis)[0].as_matrix()
			)

		vertices, faces = polyhedra.calcConeMesh(
			(0, 0, 0),
			self.data["beam_height"],
		...
		generic_cone_points = polyhedra.calcConePoints(
			(0, 0, 0),
			self.data["beam_height"],
			self.data["start_beam_vec"],
			self.data["beam_angle_deg"],
		...
		self.data["generic_circle_points"] = generic_cone_points[
			(generic_cone_points != np.asarray((0, 0, 0))).all(axis=1), :
		]
		self.data["generic_circle_points"] = np.vstack(
			(self.data["generic_circle_points"], self.data["generic_circle_points"][0, :])
		)
		circles = self._genBeamCircles(instance_transforms, instance_positions)
		self.visuals["circles"] = vVisuals.Line(
			circles,
			connect=self.data["c_conn"],
			color=colours.normaliseColour(
				(
					self.opts["beams_colour"]["value"][0] / 2,
		...
		self.visuals["scircle"] = vVisuals.Line(
			circles,
		...
		self.visuals["beams"] = vVisuals.InstancedMesh(
			vertices,
			faces,
	...
	def mouseOver(self, index: int) -> Self:
		self.data["s_c_conn"] = (
			np.array(
				[
					np.arange(self.data["num_generic_circle_points"] - 1),
					np.arange(1, self.data["num_generic_circle_points"]),
				]
			).T
			+ index * self.data["num_generic_circle_points"]
		)
	...
	def setBeamsColour(self, new_colour: tuple[int, int, int]) -> None:
		logger.debug(
			"Changing instanced beams colour %s -> %s",
			self.opts["beams_colour"]["value"],
			new_colour,
		)
		self.opts["beams_colour"]["value"] = new_colour
		if self.data["num_sats"] > 0:
			new_cols_array = vcolor_array.ColorArray(
				[colours.normaliseColour(self.opts["beams_colour"]["value"])]
				* self.data["num_sats"]
			)
	...
	def setBeamsAlpha(self, alpha: float) -> None:
		logger.debug(
			"Changing instanced beams alpha %s -> %s", self.opts["beams_alpha"]["value"], alpha
		)
	...
	def _genBeamCircles(
		self,
		instance_transforms: list[nptyping.NDArray] | nptyping.NDArray,
		instance_positions: list[nptyping.NDArray] | nptyping.NDArray,
	) -> nptyping.NDArray | None:
		total_len = 0
		gen_conn = False
		circles = None
		...
		for ii in range(len(instance_transforms)):
			circle = (
				np.dot(instance_transforms[ii], self.data["generic_circle_points"].T).T
				+ instance_positions[ii]
			)
			poly_len = len(circle)
...
class ConstellationBeams(base_assets.AbstractAsset):
	...
	def _createVisuals(self) -> None:
		...
		for ii in range(self.data["num_sats"]):
			...
			self.visuals["beams"].append(
				vVisuals.Mesh(vertices, faces, color=instance_colours[ii, :], parent=None)
			)
			T = np.eye(4)
	...
	def setBeamsColour(self, new_colour: tuple[float, float, float]) -> None:
		logger.debug(
			"Changing beams colour %s -> %s", self.opts["beams_colour"]["value"], new_colour
		)
		self.opts["beams_colour"]["value"] = new_colour
		if self.data["num_sats"] > 0:
			for ii in range(self.data["num_sats"]):
				...
				self.visuals["beams"]._meshdata.set_face_colors(
					np.tile(colours.normaliseColour(new_colour), (n_faces, 1))
				)
				self.visuals["beams"]._meshdata.set_vertex_colors(
