...
class Orbit3DAsset(base_assets.AbstractAsset):
	...
	def _createVisuals(self) -> None:
		self.visuals["past"] = scene.visuals.Line(
			self.data["past_coords"],
			color=colours.normaliseColour(self.opts["orbital_path_colour"]["value"]),
			antialias=True,
			width=self.opts["orbital_path_width"]["value"],
			parent=None,
		)
		self.visuals["future"] = scene.visuals.Line(
			self.data["future_coords"],
			color=colours.normaliseColour(self.opts["orbital_path_colour"]["value"]),
			antialias=True,
			width=self.opts["orbital_path_width"]["value"],
			parent=None,
	...
	def recomputeRedraw(self) -> None:
		...
		if self.isStale():
			...
			self.visuals["future"].set_data(
				pos=self.data["future_coords"], connect=self.data["future_conn"]
			)
	...
	def _setDefaultOptions(self) -> None:
		...
		self._dflt_opts["orbital_path_colour"] = {
			"value": (0, 0, 255),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["orbital_path_width"] = {
			"value": 1,
			"type": "integer",
			"help": "",
			"static": True,
		...
		self._dflt_opts["orbital_path_past_style"] = {
			"value": "solid",
			"type": "option",
			"options": ["dash", "solid"],
			"help": "",
		...
		self._dflt_opts["orbital_path_future_style"] = {
			"value": "solid",
			"type": "option",
			"options": ["dash", "solid"],
			"help": "",
		...
		self._dflt_opts["plot_orbital_path_future"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["plot_orbital_path_past"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["orbital_path_future_dash_size"] = {
			"value": 3,
			"type": "integer",
			"help": "",
			"static": True,
	...
	def setOrbitalPathWidth(self, value: int) -> None:
		...
		self.visuals["future"].set_data(
			pos=self.data["future_coords"], connect=self.data["future_conn"], width=value
		)
	...
	def _sliceData(self) -> None:
		...
		conn = (
			np.arange(padded_len - 1, -1, -1)
			.reshape(-1, dash_size * 2)[:, :dash_size]
			.reshape(1, -1)[0]
		)
...
class Orbit2DAsset(base_assets.AbstractAsset):
	...
	def recomputeRedraw(self) -> None:
		if self.isFirstDraw():
			self._clearFirstDrawFlag()
		if self.isStale():
			self.visuals["past"].set_data(
				pos=self.data["past_coords"], connect=self.data["past_conn"]
			)
			self.visuals["future"].set_data(
				pos=self.data["future_coords"], connect=self.data["future_conn"]
			)
	...
	def setOrbitalPathWidth(self, value: int) -> None:
		self.opts["orbital_path_width"]["value"] = value
		self.visuals["past"].set_data(
			pos=self.data["past_coords"], connect=self.data["past_conn"], width=value
		)
		self.visuals["future"].set_data(
			pos=self.data["future_coords"], connect=self.data["future_conn"], width=value
		)
