...
class BodyGizmo(base_assets.AbstractSimpleAsset):
	...
	def _createVisuals(self) -> None:
		self.visuals["gizmo"] = scene.visuals.XYZAxis(
			width=self.opts["gizmo_width"]["value"], parent=None
		)
	...
	def _setDefaultOptions(self) -> None:
		...
		self._dflt_opts["gizmo_X_axis_colour"] = {
			"value": (255, 0, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["gizmo_Y_axis_colour"] = {
			"value": (0, 255, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["gizmo_Z_axis_colour"] = {
			"value": (0, 0, 255),
			"type": "colour",
			"help": "",
			"static": True,
	...
	def restoreGizmoColours(self) -> None:
		old_colour_arr = self.visuals["gizmo"].color
		old_colour_arr[0, 0:3] = np.asarray(
			colours.normaliseColour(self.opts["gizmo_X_axis_colour"]["value"])
		)
		old_colour_arr[1, 0:3] = np.asarray(
			colours.normaliseColour(self.opts["gizmo_X_axis_colour"]["value"])
		)
		old_colour_arr[2, 0:3] = np.asarray(
			colours.normaliseColour(self.opts["gizmo_Y_axis_colour"]["value"])
		)
		old_colour_arr[3, 0:3] = np.asarray(
			colours.normaliseColour(self.opts["gizmo_Y_axis_colour"]["value"])
		)
		old_colour_arr[4, 0:3] = np.asarray(
			colours.normaliseColour(self.opts["gizmo_Z_axis_colour"]["value"])
		)
		old_colour_arr[5, 0:3] = np.asarray(
			colours.normaliseColour(self.opts["gizmo_Z_axis_colour"]["value"])
		)
		self.visuals["gizmo"].set_data(color=old_colour_arr)
...
class ViewBoxGizmo(base_assets.AbstractSimpleAsset):
	...
	def _createVisuals(self) -> None:
		self.visuals["origin"] = scene.visuals.Markers(
			pos=np.array([[0, 0, 0]]).reshape(1, 3),
			edge_width=0,
		...
		t_origin = vTransforms.STTransform(
		...
		self.visuals["gizmo"] = scene.visuals.XYZAxis(
			parent=None, width=self.opts["gizmo_width"]["value"]
		)
		self._updateAxesLengths()
		t_gizmo = vTransforms.STTransform(
			translate=self.location,
			scale=(
	...
	def _setViewBoxPosition(self) -> None:
		...
		self.visuals["gizmo"].transform.scale(
			(self.opts["gizmo_scale"]["value"], self.opts["gizmo_scale"]["value"], 0.001)
		)
		self.visuals["gizmo"].transform.translate((self.location[0], self.location[1]))
		self.visuals["origin"].transform.scale(
			(self.opts["origin_scale"]["value"], self.opts["origin_scale"]["value"], 0.001)
		)
	...
	def _setDefaultOptions(self) -> None:
		...
		self._dflt_opts["gizmo_location"] = {
			"value": "bottom_left",
			"type": "option",
