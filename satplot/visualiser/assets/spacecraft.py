...
class Spacecraft3DAsset(base_assets.AbstractAsset):
	...
	def setSource(self, *args, **kwargs) -> None:
		...
		if type(args[0]) is not data_types.SpacecraftConfig:
			logger.error(
				"setSource() of %s requires a %s as args[0], not: %s",
				self,
				data_types.SpacecraftConfig,
				type(args[0]),
			)
			raise TypeError(
				f"setSource() of {self} requires a {data_types.SpacecraftConfig} as args[0], not: {type(args[0])}"
			)
		...
		pointing_active_changed = (
			self.data["history_src"].getConfigValue("is_pointing_defined")
			!= self.data["old_pointing_defined"]
		)
		...
		if (not pointing_defined and pointing_active_changed) or (
			pointing_defined and config_changed
		):
		...
		if self.data["history_src"]:
			self.data["old_pointing_defined"] = self.data["history_src"].getConfigValue(
				"is_pointing_defined"
			)
		else:
			self.data["old_pointing_defined"] = False

		self.setOrbitalMarkerVisibility(
			not self.data["history_src"].getConfigValue("is_pointing_defined")
		)
		self.setAttitudeAssetsVisibility(
			self.data["history_src"].getConfigValue("is_pointing_defined")
		)

	def _instantiateAssets(self) -> None:
		self.assets["body_frame"] = gizmo.BodyGizmo(
			scale=700, width=3, v_parent=self.data["v_parent"]
		)
	...
	def _instantiateSensorAssets(self) -> None:
		for suite_name, suite_config in self.data["sc_config"].getSensorSuites().items():
			...
			self.assets[f"sensor_suite_{suite_name}"] = sensors.SensorSuite3DAsset(
				self.data["sc_config"].id,
				suite_config,
				name=suite_name,
				v_parent=self.data["v_parent"],
			)
			self.assets[f"sensor_suite_{suite_name}"].setVisibility(False)
	...
	def recomputeRedraw(self) -> None:
		...
		if self.isStale():
			...
			self.data["curr_pos"] = (
				...
				.getOrbits()[self.data["sc_config"].id]
			...
			if self.data["history_src"].getConfigValue("is_pointing_defined"):
				...
				rot_mat = (
					self.data["history_src"]
					.getSCAttitude(self.data["sc_config"].id)
					.getAttitudeMatrix(self.data["curr_index"])
				)
				quat = (
					self.data["history_src"]
					.getSCAttitude(self.data["sc_config"].id)
					.getAttitudeQuat(self.data["curr_index"])
				)
				# recomputeRedraw child assets
				self.data["curr_quat"] = quat
				self._recomputeRedrawChildren(
					pos=self.data["curr_pos"].reshape(1, 3), rotation=rot_mat
				)
			else:
				self._recomputeRedrawChildren(
					pos=self.data["coords"][self.data["curr_index"]].reshape(1, 3)
	...
	def _setDefaultOptions(self) -> None:
		self._dflt_opts = {}
		self._dflt_opts["spacecraft_marker_colour"] = {
			"value": (255, 0, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["plot_spacecraft_marker"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["spacecraft_marker_size"] = {
			"value": 500,
			"type": "number",
			"help": "",
			"static": True,
		...
		self._dflt_opts["plot_body_frame"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["plot_all_sensor_suites"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
	...
	def _addIndividualSensorSuitePlotOptions(self) -> None:
		...
		for key in self.data["sc_config"].getSensorSuites().keys():
			...
			self.opts[f"plot_sensor_suite_{key}"] = {
				"value": True,
				"type": "boolean",
				"help": "",
				"static": False,
	...
	def _removeOldSensorSuitePlotOptions(self, old_suite_names: list[str]) -> None:
		for suite_name in old_suite_names:
			if self.opts[f"plot_sensor_suite_{suite_name}"]["widget_data"] is not None:
				self.opts[f"plot_sensor_suite_{suite_name}"]["widget_data"]["mark_for_removal"] = (
					True
				)
			self.assets[f"sensor_suite_{suite_name}"].removePlotOptions()
...
class Spacecraft2DAsset(base_assets.AbstractAsset):
	...
	def setSource(self, *args, **kwargs) -> None:
		...
		if type(args[2]) is not earth_raycast_data.EarthRayCastData:
			logger.error(
				"setSource() of %s requires a %s as args[2], not: %s",
				self,
				earth_raycast_data.EarthRayCastData,
				type(args[2]),
			)
			raise TypeError(
				f"setSource() of {self} requires a {earth_raycast_data.EarthRayCastData} as args[2], not: {type(args[2])}"
			)
		...
		if hasattr(first_sat_orbit, "pos"):
			self.data["coords"] = np.hstack(
				(first_sat_orbit.lon.reshape(-1, 1), first_sat_orbit.lat.reshape(-1, 1))
			)
	...
	def _instantiateSensorAssets(self) -> None:
		for suite_name, suite_config in self.data["sc_config"].getSensorSuites().items():
			logger.info("Creating sensor suite sensor_suite_%s", suite_name)
			self.assets[f"sensor_suite_{suite_name}"] = sensors.SensorSuite2DAsset(
				self.data["sc_config"].id,
				suite_config,
				name=suite_name,
				v_parent=self.data["v_parent"],
	...
	def _createVisuals(self) -> None:
		self.visuals["marker"] = scene.visuals.Markers(parent=None, scaling=True, antialias=0)
		self.visuals["marker"].set_data(
			pos=self.data["coords"][self.data["curr_index"]].reshape(1, 2),
			edge_width=0,
			face_color=colours.normaliseColour(self.opts["spacecraft_marker_colour"]["value"]),
			edge_color="white",
			size=self.opts["spacecraft_marker_size"]["value"],
		...
		self.visuals["oth_circle1"] = polygon_visuals.FastPolygon(
			self.data["oth_edge1"],
			color=colours.normaliseColour(self.opts["over_the_horizon_circle_colour"]["value"]),
			border_color=colours.normaliseColour(
				self.opts["over_the_horizon_circle_colour"]["value"]
			),
			border_width=2,
			parent=None,
		)
		...
		self.visuals["oth_circle2"] = polygon_visuals.FastPolygon(
			self.data["oth_edge2"],
			color=colours.normaliseColour(self.opts["over_the_horizon_circle_colour"]["value"]),
			border_color=colours.normaliseColour(
				self.opts["over_the_horizon_circle_colour"]["value"]
	...
	def recomputeRedraw(self) -> None:
		...
		if self.isStale():
			...
			if self.data["history_src"].getConfigValue("is_pointing_defined"):
				...
				for asset_name, asset in self.assets.items():
					if "sensor_suite_" in asset_name:
						asset.setCurrentDatetime(
							self.data["history_src"].timespan[self.data["curr_index"]]
						)
				# recomputeRedraw child assets
				self.data["curr_quat"] = quat
				self._recomputeRedrawChildren(
			...
			if split:
				self.visuals["oth_circle1"].opacity = self.opts["over_the_horizon_circle_alpha"][
					"value"
				]
				self.visuals["oth_circle2"].opacity = self.opts["over_the_horizon_circle_alpha"][
					"value"
				]
				# self.visuals['oth_circle1'].opacity = 1
				# self.visuals['oth_circle2'].opacity = 1
			else:
				self.visuals["oth_circle1"].opacity = (
					self.opts["over_the_horizon_circle_alpha"]["value"] / 2
				)
				self.visuals["oth_circle2"].opacity = (
					self.opts["over_the_horizon_circle_alpha"]["value"] / 2
				)
			self._clearStaleFlag()
	...
	def _setDefaultOptions(self) -> None:
		...
		self._dflt_opts["plot_over_the_horizon_circle"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["over_the_horizon_circle_colour"] = {
			"value": (255, 234, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["over_the_horizon_circle_alpha"] = {
			"value": 0.4,
			"type": "fraction",
			"help": "",
			"static": True,
	...
	def setOTHCircleAlpha(self, alpha):
		...
		logger.debug(
			"Changing terminator alpha %s -> %s",
			self.opts["over_the_horizon_circle_alpha"]["value"],
			alpha,
		)
...
class SpacecraftViewsAsset(base_assets.AbstractAsset):
	...
	def _instantiateSensorAssets(self) -> None:
		for suite_name, suite_config in self.data["sc_config"].getSensorSuites().items():
			logger.info("Creating sensor suite sensor_suite_%s", suite_name)
			self.assets[f"sensor_suite_{suite_name}"] = sensors.SensorSuiteImageAsset(
				self.data["sc_config"].id,
				suite_config,
				name=suite_name,
				v_parent=self.data["v_parent"],
	...
	def recomputeRedraw(self) -> None:
		...
		if self.isStale():
			if self.data["history_src"].getConfigValue("is_pointing_defined"):
				...
				self.data["curr_quat"] = quat
				self._recomputeRedrawChildren(
					pos=orbit_data.pos[self.data["curr_index"]].reshape(1, 3), rotation=rot_mat
				)
			self._clearStaleFlag()
