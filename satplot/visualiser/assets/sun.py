...
class Sun3DAsset(base_assets.AbstractAsset):
	...
	def _initData(self):
		...
		self.data["umbra_vertices"], self.data["umbra_faces"] = polyhedra.calcCylinderMesh(
			(0, 0, 0),
		...
		self.data["vector"] = np.asarray([[0, 0, 0, 1], [c.R_EARTH / 4, 0, 0, 1]])
		self.data["vector_head_vertices"], self.data["vector_head_faces"] = polyhedra.calcConeMesh(
			(0, 0, 0), 500, self.data["vector_start_axis"], 45, axis_sample=2, theta_sample=30
		)
	...
	def _createSunVisual(self):
		# Sun Sphere
		self.visuals["sun_sphere"] = scene.visuals.Sphere(
			radius=self.opts["sun_sphere_radius_kms"]["value"],
			method="latitude",
	...
	def _createUmbraVisual(self):
		# Umbra
		self.visuals["umbra"] = vVisuals.Mesh(
			self.data["umbra_vertices"],
	...
	def _createVectorVisual(self):
		# Sun Vector
		self.visuals["vector_body"] = vVisuals.Line(
			self.data["vector"][:, 0:3],
			color=colours.normaliseColour(self.opts["sun_vector_colour"]["value"]),
			width=self.opts["sun_vector_width"]["value"],
			parent=None,
		)

		self.visuals["vector_head"] = vVisuals.Mesh(
			self.data["vector_head_vertices"],
	...
	def setSource(self, *args, **kwargs):
		...
		if type(first_sat_orbit) is not orbit.Orbit:
			logger.error(
				"data source for %s is not an orbit.Orbit, can't extract sun location data", self
			)
			raise TypeError
	...
	def recomputeRedraw(self):
		...
		if self.isStale():
			...
			rot_mat = Rotation.align_vectors(self.data["umbra_start_axis"], umbra_dir)[
				0
			].as_matrix()
			t_mat1 = np.eye(4)
			...
			# move sun vector
			sun_vec_start = (
				np.linalg.norm(sun_pos) - self.opts["sun_vector_length_kms"]["value"]
			) * pg.unitVector(sun_pos)
			arrow_head_point = (
				np.linalg.norm(sun_pos) - self.opts["sun_vector_length_kms"]["value"] - 500
			) * pg.unitVector(sun_pos)
			sun_vec_end = sun_pos
			t_mat2 = np.eye(4)
			t_mat2[0:3, 0:3] = -rot_mat
			t_mat2[3, 0:3] = arrow_head_point
			new_vec = np.vstack((sun_vec_start, sun_vec_end))
	...
	def _setDefaultOptions(self):
		...
		self._dflt_opts["plot_sun_sphere"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["sun_sphere_colour"] = {
			"value": (255, 162, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["plot_sun_vector"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["sun_vector_colour"] = {
			"value": (255, 162, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["sun_vector_width"] = {
			"value": 1,
			"type": "float",
			"help": "",
			"static": True,
		...
		self._dflt_opts["sun_vector_length_kms"] = {
			"value": c.R_EARTH / 3,
			"type": "integer",
			"help": "",
			"static": True,
		...
		self._dflt_opts["sun_distance_kms"] = {
			"value": 15000,
			"type": "number",
			"help": "",
			"static": True,
		...
		self._dflt_opts["sun_sphere_radius_kms"] = {
			"value": 786,
			"type": "number",
			"help": "",
			"static": True,
	...
	def _updateLineVisualsOptions(self):
		...
		self.visuals["vector_body"].set_data(
			color=new_colour, width=self.opts["sun_vector_width"]["value"]
		)
	...
	def setSunSphereColour(self, new_colour):
		logger.debug(
			"Changing sun sphere colour %s -> %s",
			self.opts["sun_sphere_colour"]["value"],
			new_colour,
		)
		...
		self.visuals["sun_sphere"].mesh._meshdata.set_face_colors(
			np.tile(colours.normaliseColour(new_colour), (n_faces, 1))
		)
		self.visuals["sun_sphere"].mesh._meshdata.set_vertex_colors(
			np.tile(colours.normaliseColour(new_colour), (n_verts, 1))
		)
	...
	def setUmbraColour(self, new_colour):
		logger.debug(
			"Changing umbra colour %s -> %s", self.opts["umbra_colour"]["value"], new_colour
		)
		...
		self.visuals["umbra"]._meshdata.set_face_colors(
			np.tile(colours.normaliseColour(new_colour), (n_faces, 1))
		)
		self.visuals["umbra"]._meshdata.set_vertex_colors(
			np.tile(colours.normaliseColour(new_colour), (n_verts, 1))
		)
	...
	def setSunVectorColour(self, new_colour):
		logger.debug(
			"Changing sun vector colour %s -> %s",
			self.opts["sun_vector_colour"]["value"],
			new_colour,
		)
		...
		self.visuals["vector_head"]._meshdata.set_face_colors(
			np.tile(colours.normaliseColour(new_colour), (n_faces, 1))
		)
		self.visuals["vector_head"]._meshdata.set_vertex_colors(
			np.tile(colours.normaliseColour(new_colour), (n_verts, 1))
		)
...
class Sun2DAsset(base_assets.AbstractAsset):
	...
	def _createVisuals(self):
		# Sun Sphere
		self.visuals["marker"] = scene.visuals.Markers(parent=None, scaling=True, antialias=0)
		self.visuals["marker"].set_data(
			pos=self.data["scaled_coords"][self.data["curr_index"]].reshape(1, 2),
			edge_width=0,
		...
		self.visuals["terminator"] = polygon_visuals.FastPolygon(
			self.data["terminator_edge"],
			color="#000000",
			border_color="#000000",
			border_width=0,
			parent=None,
		...
		self.visuals["eclipse_patch1"] = polygon_visuals.FastPolygon(
			self.data["eclipse_edge"],
			color="#000000",
			border_color="#000000",
			border_width=0,
			parent=None,
		...
		self.visuals["eclipse_patch2"] = polygon_visuals.FastPolygon(
			self.data["eclipse_edge"],
			color="#000000",
			border_color="#000000",
			border_width=0,
	...
	def setSource(self, *args, **kwargs):
		...
		if type(args[0]) is not history_data.HistoryData:
			logger.error(
				"setSource() of %s requires a %s as args[1], not: %s",
				self,
				history_data.HistoryData,
				type(args[1]),
			)
			raise TypeError(
				f"setSource() of {self} requires a {history_data.HistoryData} as args[1], not: {type(args[1])}"
			)
			return
	...
	def recomputeRedraw(self):
		...
		if self.isStale():
			# move the sun
			self._updateMarkers()
			terminator_boundary = self.calcTerminatorOutline(
				self.data["coords"][self.data["curr_index"]]
			)

			self.data["terminator_edge"][:, 0] = (terminator_boundary[:, 0] + 180) * self.data[
				"horiz_pixel_scale"
			]
			self.data["terminator_edge"][:, 1] = (terminator_boundary[:, 1] + 90) * self.data[
				"vert_pixel_scale"
			]

			self.visuals["terminator"].pos = self.data["terminator_edge"]

			self.data["eclipse_edge1"], self.data["eclipse_edge2"], split = self.calcEclipseOutline(
				self.data["coords"][self.data["curr_index"]],
				np.linalg.norm(self.data["coords"][self.data["curr_index"]]),
			)
	...
	def _setDefaultOptions(self):
		...
		self._dflt_opts["plot_sun_marker"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["sun_marker_colour"] = {
			"value": (255, 162, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["plot_terminator"] = {
			"value": True,
			"type": "boolean",
			"help": "",
			"static": True,
		...
		self._dflt_opts["terminator_colour"] = {
			"value": (0, 0, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["eclipse_colour"] = {
			"value": (0, 0, 0),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["terminator_alpha"] = {
			"value": 0.4,
			"type": "fraction",
			"help": "",
			"static": True,
		...
		self._dflt_opts["sun_marker_size"] = {
			"value": 30,
			"type": "number",
			"help": "",
			"static": True,
	...
	def setSunMarkerColour(self, new_colour):
		logger.debug(
			"Changing sun marker colour %s -> %s",
			self.opts["sun_marker_colour"]["value"],
			new_colour,
		)
	...
	def setSunMarkerSize(self, size):
		logger.debug(
			"Changing sun marker size %s -> %s", self.opts["sun_marker_size"]["value"], size
		)
	...
	def setTerminatorAlpha(self, alpha):
		# Takes a little while to take effect.
		logger.debug(
			"Changing terminator alpha %s -> %s", self.opts["terminator_alpha"]["value"], alpha
		)
	...
	def setTerminatorColour(self, new_colour):
		logger.debug(
			"Changing terminator colour %s -> %s",
			self.opts["terminator_colour"]["value"],
			new_colour,
		)
	...
	def setEclipseColour(self, new_colour):
		logger.debug(
			"Changing eclipse colour %s -> %s", self.opts["eclipse_colour"]["value"], new_colour
		)
	...
	def getTerminatorPoint(
		self, earth_lon: float | np.ndarray, solar_lat: float, solar_lon: float
	) -> float:
	...
	def calcTerminatorOutline(self, solar_lonlat) -> np.ndarray:
		...
		terminator_boundary[:, 1] = np.rad2deg(
			self.getTerminatorPoint(
				np.deg2rad(terminator_boundary[:, 0]), np.deg2rad(solar_lat), np.deg2rad(solar_lon)
			)
		)
	...
	def calcEclipseOutline(self, solar_lonlat, sat_altitude) -> tuple[np.ndarray, np.ndarray, bool]:
		...
		eclipse_center_lon = np.rad2deg(
			spherical_geom.wrapToCircleRange(np.deg2rad(solar_lonlat[0] + 180))
		)
		phi_h = np.rad2deg(np.arcsin(c.R_EARTH / (c.R_EARTH + sat_altitude)))
		lats, lons1, lons2 = spherical_geom.genSmallCircleCenterSubtendedAngle(
			phi_h * 2, eclipse_center_lat, eclipse_center_lon
		)
		circle1, circle2 = spherical_geom.splitSmallCirclePatch(
			eclipse_center_lon, eclipse_center_lat, lats, lons1, lons2
		)
