...
class Events3DAsset(base_assets.AbstractAsset):
	...
	def setSource(self, *args, **kwargs):
		...
		if type(args[0]) is not event_data.EventData:
			...
			raise TypeError(
				f"setSource() of {self} requires a {event_data.EventData} as args[1], not: {type(args[1])}"
			)
	...
	def getScreenMouseOverInfo(self) -> dict[str, Any]:
		...
		mo_info["screen_pos"] = [
			(canvas_pos[ii, 0], canvas_pos[ii, 1]) for ii in range(len(self.data["coords"]))
		]
		mo_info["world_pos"] = [
			self.data["coords"][ii, :].reshape(3) for ii in range(len(self.data["coords"]))
		]
	...
	def _setDefaultOptions(self):
		...
		self._dflt_opts["prior_events_marker_colour"] = {
			"value": (156, 0, 252),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["post_events_marker_colour"] = {
			"value": (80, 60, 125),
			"type": "colour",
			"help": "",
			"static": True,
		...
		self._dflt_opts["events_marker_size"] = {
			"value": 200,
			"type": "number",
			"help": "",
			"static": True,
		...
		self._dflt_opts["events_marker_style"] = {
			"value": "disc",
			"type": "option",
			"options": [
				"disc",
	...
	def _updateMarkers(self):
		cols = np.tile(
			colours.normaliseColour(self.opts["prior_events_marker_colour"]["value"]),
			(len(self.data["coords"]), 1),
		)
		...
		self.visuals["marker"].set_data(
			pos=self.data["coords"].reshape(-1, 3),
			size=self.opts["events_marker_size"]["value"],
			face_color=cols,
			symbol=self.opts["events_marker_style"]["value"],
		)
	...
	def setPriorEventsMarkerColour(self, new_colour):
		logger.debug(
			"Changing events prior marker colour %s -> %s",
			self.opts["prior_events_marker_colour"]["value"],
			new_colour,
		)
	...
	def setPostEventsMarkerColour(self, new_colour):
		logger.debug(
			"Changing events post marker colour %s -> %s",
			self.opts["post_events_marker_colour"]["value"],
			new_colour,
		)
	...
	def setEventsMarkerSize(self, size):
		logger.debug(
			"Changing events marker size %s -> %s", self.opts["events_marker_size"]["value"], size
		)
	...
	def setMarkerStyle(self, option_idx):
		...
		logger.debug(
			"Changing events marker style %s -> %s",
			self.opts["events_marker_style"]["value"],
			new_style,
		)
...
class Events2DAsset(base_assets.AbstractAsset):
	...
	def setSource(self, *args, **kwargs):
		...
		scaled_lat = ((self.data["coords"][:, 0] + 90) * self.data["vert_pixel_scale"]).reshape(
			-1, 1
		)
		scaled_lon = ((self.data["coords"][:, 1] + 180) * self.data["horiz_pixel_scale"]).reshape(
			-1, 1
		)
	...
	def _updateMarkers(self):
		...
		cols[post_truth] = colours.normaliseColour(self.opts["post_events_marker_colour"]["value"])
		self.visuals["marker"].set_data(
			pos=self.data["scaled_coords"].reshape(-1, 2),
			size=self.opts["events_marker_size"]["value"],
			face_color=cols,
			symbol=self.opts["events_marker_style"]["value"],
		)
