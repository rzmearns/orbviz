...
class Application(QtWidgets.QApplication):
	...
	def _connectAllContextControls(self, context: BaseCanvas) -> None:
		if context.controls is not None:
			...
			context.controls.action_dict["spacetrak-credentials"]["callback"] = (
				dialogs.SpaceTrackCredentialsDialog
			)
			context.controls.action_dict["groundstations"]["callback"] = (
				self.window.setActiveShellGroundStations
			)
		else:
			logger.error(
				"context: %s does not have an associated action dictionary from a resources/actions/<context>.json file.",
				context,
			)
			raise ValueError()
	...
	def saveAs(self) -> None:
		...
		save_file = self._openFileDialog(
			"SatPlot Save...", pathlib.Path("data/saves/"), dflt_save_file
		)
		if save_file.name != "":
	...
	def _saveState(self) -> None:
		...
		if self.save_file is not None:
			with self.save_file.open("wb") as picklefp:
	...
	def load(self) -> None:
		load_file = self._openFileDialog(
			"SatPlot Load...", pathlib.Path("data/saves/"), None, save=False
		)
		if load_file != "":
	...
	def _loadState(self, load_file: pathlib.Path) -> None:
		...
		with load_file.open("rb") as picklefp:
			state = pickle.load(picklefp)
		self.deSerialise(state)

	def _openFileDialog(
		self, caption: str, dflt_dir: pathlib.Path, dflt_filename: str | None, save=True
	) -> pathlib.Path:
		...
		if save:
			filename, _ = QtWidgets.QFileDialog.getSaveFileName(
				None, caption, f"{dflt_dir}{dflt_filename}", "pickles (*.pickle)", options=options
			)
		else:
			filename, _ = QtWidgets.QFileDialog.getOpenFileName(
				None, caption, f"{dflt_dir}", "pickles (*.pickle)", options=options
			)
	...
	def deSerialise(self, state: dict[str, Any]) -> None:
		if state["metadata"]["version"] != satplot.version:
			logger.error(
				"This satplot state was not created with this version of satplot: file %s, satplot %s",
		...
		if state["metadata"]["gl_plus"] != satplot.gl_plus:
			logger.error(
				"WARNING: this file was created with a different GL mode: GL+ = %s. It may not load correctly.",
				satplot.gl_plus,
			)
