...
class SensorSuite3DAsset(base_assets.AbstractCompoundAsset):
	...
	def _instantiateAssets(self) -> None:
		...
		for sensor in sensor_names:
			...
			if sens_dict['shape'] == orbviz_data_types.SensorTypes.CONE:
				self.assets[sensor] = Sensor3DAsset.cone(self.data['sc_id'],
															sensor,
															self.data['name'],
															sens_dict,
															parent=self.data['v_parent'])
			elif sens_dict['shape'] == orbviz_data_types.SensorTypes.FPA:
...
class Sensor3DAsset(base_assets.AbstractSimpleAsset):
	def __init__(self, sc_id:int, sensor_name:str, parent_suite_name:str, mesh_verts, mesh_faces,
		...
		self._setDefaultOptions()
		if sens_type is None or not orbviz_data_types.SensorTypes.hasValue(sens_type):
...
class SensorImageAsset(base_assets.AbstractSimpleAsset):
	...
	def _decodeLabelData(self, data:np.ndarray) -> str:
		if data[0] == 0:
			...
			# celestial
			raH,raM,raS = orbviz_conversion.decimal2hhmmss(data[1])
			decD,decM,decS = orbviz_conversion.decimal2degmmss(data[2])
