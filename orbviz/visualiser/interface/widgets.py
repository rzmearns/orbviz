...
class FilePicker(QtWidgets.QWidget):
	def __init__(self, label,
		...
		# Create widgets
		path_to_show = self.path.relative_to(orbviz_paths.data_dir)
	...
	def _setPath(self):
		self.path = pathlib.Path(orbviz_paths.data_dir.joinpath(self._file_text_box.text()))
...
class PrimaryConfigDisplay(QtWidgets.QWidget):
	...
	def updateConfig(self, config:orbviz_data_types.PrimaryConfig):
		self.config = config
...
class ConstellationConfigDisplay(QtWidgets.QWidget):
	...
	def updateConfig(self, config:orbviz_data_types.ConstellationConfig):
		self.config = config
		self._cnfg_table.setItem(0,1,QtWidgets.QTableWidgetItem(f'{self.config.filestem}.json'))
		self._cnfg_table.setItem(1,1,QtWidgets.QTableWidgetItem(f'{self.config.name}'))
